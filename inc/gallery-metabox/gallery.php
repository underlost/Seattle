...
function gallery_meta_callback($post) {
  ...
  $ids = get_post_meta($post->ID, 'seattle_gallery_id', true);
  ?>
    <table class="form-table">
      <tr><td>
        ...
        <?php if ($ids):
          ...
          <li>
            <input type="hidden" name="seattle_gallery_id[<?php echo $key; ?>]" value="<?php echo $value; ?>">
...
function gallery_meta_save($post_id) {
  ...
  if (isset($_POST['seattle_gallery_id'])) {
    update_post_meta($post_id, 'seattle_gallery_id', $_POST['seattle_gallery_id']);
  } else {
    delete_post_meta($post_id, 'seattle_gallery_id');
  }
}
